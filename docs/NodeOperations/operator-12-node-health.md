...
sudo ufw status
````
Make sure you have opened for ports 9888-9897. If not consult instructions [here](/docs/NodeOperations/operator-4-security.md).
...
Running two nodes with same config can be interpreted as malicious behavior.You can start a [reader node](/docs/NodeOperations/operator-5-reader-vps.md) on the new VPS. Then, when you are ready to change the `config.json` to the BP version, you stop the node from running on the old server:
