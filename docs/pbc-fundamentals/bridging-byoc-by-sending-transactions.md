...
PBC nodes can read information on the native chains of the cryptocurrencies used for BYOC, e.g. a PBC node has an Ethereum endpoint, meaning the nodes on PBC can read when a deposit have been made to a contract on Ethereum, but Ethereum nodes do not read information on PBC. So, when the  needs to provide information from 
...
3. The action _deposit_ is invoked on [ETH Deposit](https://browser.partisiablockchain.com/contracts/045dbd4c13df987d7fb4450e54bcd94b34a80f2351) by the oracle, given that there are enough valid signatures the contract mints X BYOC twins minus a fee of 0.1% of transferred amount  
