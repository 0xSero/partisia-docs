...
You run the node from the folder `/opt/pbc-mainnet` with user:group `1500:1500`. First we need to create the `conf`
and `storage` folders for the application:
...
If you have made any changes to the template, then check if your file is
still [valid yml-format](https://www.yamllint.com/).
