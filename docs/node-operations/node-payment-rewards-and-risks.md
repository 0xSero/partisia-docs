...
When tokens are associated to a contract and you (the node operator) want to disassociate the tokens from the service/contract there is some pending time from both
...
| stakedToContract ([Block Producer Orchestration ](https://browser.partisiablockchain.com/contracts/04203b77743ad0ca831df9430a6be515195733ad91))  | 28                  | If you are in the committee when you invoke [Remove bp](https://browser.partisiablockchain.com/contracts/04203b77743ad0ca831df9430a6be515195733ad91/removeBp) , the node will serve in committee until rotation comes changes by itself, or you can manually trigger a committee change when the committee is at least 28 days old. If your node is not in current committee, you will get the tokens disassociated from the contract immediately when invoking _Remove bp_.              | [Remove bp](https://browser.partisiablockchain.com/contracts/04203b77743ad0ca831df9430a6be515195733ad91/removeBp) the command removes you as a baker node and disassociates tokens from the contract                                                                                                            |
...
| stakedToContract (Any price oracle, [find out which one your node serve](run-a-price-oracle-node.md#how-to-deregister-as-a-price-oracle))                                                                                    | 0-1                 | You have to deregister outside a challenge period. If you do that disassociation is immediate.                                                                                                                                                          | [Deregister](run-a-price-oracle-node.md#deregister-from-a-price-oracle)                                                                                                                                       |
...
- If delegated to the account of a node operator, but not associated to be usable by a contract for a job, owner can retrieve with no issues
- If delegated to an account of a node operator, and associated with a job but not being used, owner cannot retrieve without the node operator disassociating the token from the job
- If delegated to the account of a node operator, associated with a job and being used, owner cannot retrieve until the job finish, pending period is over node operator disassociating the token from the job
- When tokens are retrieved from delegation by the owner they will not be transferable even if passed vesting schedule until unstaked by the owner
- Only tokens controlled by your account can be fully unstaked, meaning change state back from staked 7 days pending minimum
