...
To update a config, the easiest way is to simply run the [node config tool](../node-operations/run-a-reader-node.md#the-node-registersh-script) again. Alternatively, `config.json` can be edited manually as shown in the example above.
